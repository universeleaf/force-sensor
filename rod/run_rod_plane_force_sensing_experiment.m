...
if ischar(quickMode) && strcmpi(quickMode, 'render')
    saved = load(overrides, 'results');
    results = saved.results;
    results.config.rootDir = rootDir;
    results.config.outputDir = fileparts(char(overrides));
...
fprintf('Full constrained EKF-MAP final total-load error: %.2f%%\n', ...
    results.metrics.ours.finalRelativeErrorPct);
fprintf('Full constrained EKF-MAP total-load RMSE over loaded frames: %.3f N\n', ...
...
cfg = struct;
cfg.rootDir = rootDir;
cfg.outputDir = fullfile(rootDir, 'out', ...
    'wall');
cfg.randomSeed = 7;
...
cfg.video.fileName = 'forces.mp4';
cfg.video.aloiFileName = 'aloi.mp4';
...
cfg.diagnostics.stopAfterInverse = false;
...
normalForceResultant = zeros(3, nt);
frictionForceResultant = zeros(3, nt);
frictionConeSlack = zeros(1, nt);
frictionWMin = zeros(1, nt);
frictionComplementarity = zeros(1, nt);
frictionDissipation = zeros(1, nt);
frictionConeViolation = zeros(1, nt);
frictionTangentialDisplacementNorm = zeros(1, nt);
frictionDirectionViolation = zeros(1, nt);
...
for internalFrame = 1:ntInternal
    ...
    if outputFrame < nt && internalFrame == sampleIdx(outputFrame + 1)
        ...
        [normalForceResultant(:, outputFrame), frictionForceResultant(:, outputFrame), ...
            frictionConeSlack(outputFrame), frictionWMin(outputFrame), ...
            frictionComplementarity(outputFrame), frictionDissipation(outputFrame), ...
            frictionConeViolation(outputFrame), frictionTangentialDisplacementNorm(outputFrame), ...
            frictionDirectionViolation(outputFrame)] = ...
            summarizeContactFriction(contacts, frictionMu(outputFrame));
...
forward.normalForceResultant = normalForceResultant;
forward.frictionForceResultant = frictionForceResultant;
forward.frictionConeSlack = frictionConeSlack;
forward.frictionWMin = frictionWMin;
forward.frictionComplementarity = frictionComplementarity;
forward.frictionDissipation = frictionDissipation;
forward.frictionConeViolation = frictionConeViolation;
forward.frictionTangentialDisplacementNorm = frictionTangentialDisplacementNorm;
forward.frictionDirectionViolation = frictionDirectionViolation;
...
e = zeros(d * nc, nc);
for ic = 1:nc
    % Use the same ordered tangent-direction set as the inverse model.
    % For even d this is identical to the upstream half-circle plus
    % negatives construction, but sharing one definition prevents a
    % forward/inverse direction-label mismatch.
    Dcur = frictionDirections(n(3 * ic - (2:-1:0), ic), d);
...
[~, xScaled, lcpStatus] = LCPSolve(Dw * M * Dx, Dw * g, 1e-8, dimM^2);
x = Dx * xScaled;
% A returned vector is not evidence that the LCP converged. Audit the
% unscaled equations at EVERY internal step, before accepting any force.
wLcp = M * x + g;
% A degenerate polyhedral LCP can report ray termination at an already
% feasible solution. The unscaled primal/complementarity residual is decisive.
if any(~isfinite([x; wLcp])) || ...
        min([x; wLcp]) < -1e-7 || max(abs(x .* wLcp)) > 1e-6
    error('force:LCPInfeasible', ...
        'LCP failed: status=%d, min variable=%.3g, complementarity=%.3g.', ...
        lcpStatus(1), min([x; wLcp]), max(abs(x .* wLcp)));
end
...
for ic = 1:nc
    if fn(ic) > 0
        ...
        contacts(ic).contact_displacement = dp(3 * ic - 2:3 * ic);
        contacts(ic).trial_displacement = q(3 * ic - 2:3 * ic);
        contacts(ic).friction_w = zeros(d, 1);
        contacts(ic).friction_force = zeros(3, 1);
        contacts(ic).friction_cone_slack = obstacles{contacts(ic).obstacle_id}.mu * fn(ic);
        ...
        if ~isempty(icFLocal)
            ...
            contacts(ic).friction_force = contacts(ic).friction_directions * ...
                contacts(ic).friction_beta;
            contacts(ic).friction_w = contacts(ic).friction_directions' * ...
                contacts(ic).contact_displacement + ...
                contacts(ic).friction_lambda * ones(d, 1);
            contacts(ic).friction_cone_slack = ...
                obstacles{contacts(ic).obstacle_id}.mu * fn(ic) - ...
                sum(contacts(ic).friction_beta);
        end
    else
...
c.contact_displacement = zeros(3, 1);
c.trial_displacement = zeros(3, 1);
c.friction_w = zeros(d, 1);
c.friction_force = zeros(3, 1);
c.friction_cone_slack = 0;
end
...
for it = 1:nt
    ...
    [fn, beta, lambdaState, reconResidual] = contactVariablesFromForwardContact(contacts, ic, contactForce, n, cfgFrame);
    x = [planePoint; eta; s1; fn; beta; lambdaState; forward.tipLoad(:, it)];
...
normalForceVector = zeros(3, nt);
frictionForceVector = zeros(3, nt);
frictionWMin = nan(1, nt);
frictionConeViolation = nan(1, nt);
tipForce = zeros(3, nt);
...
for it = 1:nt
    ...
    frictionWMin(it) = min(decoded.frictionW);
...
ours.normalForceResultant = normalForceVector;
ours.frictionForceResultant = frictionForceVector;
ours.frictionWMin = frictionWMin;
ours.frictionConeViolation = frictionConeViolation;
...
decoded = decodeMapState(x, tube, cfg, prevShape);
forceTol = 5e-2;
slipTolMm = cfg.forceSensor.slipToleranceMm;
if isstruct(prevShape) && isfield(prevShape, 'T_base')
    baseStep = tube.T_base(1:3,4) - prevShape.T_base(1:3,4);
    tangentialStep = norm((eye(3)-decoded.n*decoded.n')*baseStep);
    if tangentialStep > 1e-8
        slipTolMm = min(slipTolMm, 0.25*tangentialStep);
    end
end
...
switch mode.name
    ...
    case 'sticking-contact'
        tangentBasis = stableTangentBasis(decoded.n);
        % Sticking still requires the complete inequality part of eq. (16).
        % The previous version constrained v=0 but omitted w=D'*v+lambda*e>=0.
        c = -[decoded.frictionW(:); decoded.frictionConeSlack];
...
% Both p and the stored predecessor are world coordinates. Advecting the
% predecessor with the moving base here would subtract the commanded slip.
% Only the Jacobian linearization below uses the rigidly moved shape.
prevShapeForDisplacement = prevShape;
...
elseif isstruct(prevShapeForDisplacement)
    ...
    prevPcCenter = interpolateVectorByArc(tube.s, prevShapeForDisplacement, s1);
end
gap = n' * (pc - p1);
...
t = contact_tangent(unitVector(n, [0; 0; 1]));
end
...
saveFigure(fig, fullfile(cfg.outputDir, 'overview.png'));
...
saveFigure(fig2, fullfile(cfg.outputDir, 'forces.png'));
...
saveFigure(fig, fullfile(cfg.outputDir, 'aloi.png'));
end
...
trueNormalForce = results.forward.normalForceResultant;
trueFrictionForce = results.forward.frictionForceResultant;
estimatedNormalForce = results.ours.normalForceResultant;
estimatedFrictionForce = results.ours.frictionForceResultant;
...
trueTangent = t' * trueFrictionForce;
estimatedTangent = t' * estimatedFrictionForce;
motion = results.forward.actuationMm;
nt = numel(motion);
...
try
    for frameIdx = 1:nVideoFrames
        ...
        trueNormalForceNow = interpolateLastDimension(trueNormalForce, q);
        trueFrictionForceNow = interpolateLastDimension(trueFrictionForce, q);
        estimatedNormalForceNow = interpolateLastDimension(estimatedNormalForce, q);
        estimatedFrictionForceNow = interpolateLastDimension(estimatedFrictionForce, q);
        ...
        modeLabel = results.ours.complementarityMode{min(nt, max(1, round(q)))};
        ...
        hPlane = plotPlaneSectionXZ(cfg.planePointMm, cfg.planeNormal, ...
            'k--', 'LineWidth', 1.1);
        hTrueNormal = quiver(axShape, trueOrigin(1), trueOrigin(3), ...
            trueNormalForceNow(1) * forceScale, trueNormalForceNow(3) * forceScale, 0, ...
            'Color', [0.10 0.45 0.80], 'LineWidth', 2.0, 'MaxHeadSize', 0.8);
        hTrueFriction = quiver(axShape, trueOrigin(1), trueOrigin(3), ...
            trueFrictionForceNow(1) * forceScale, trueFrictionForceNow(3) * forceScale, 0, ...
            'Color', [0.10 0.65 0.25], 'LineWidth', 2.0, 'MaxHeadSize', 0.8);
        hEstimatedNormal = quiver(axShape, estimatedOrigin(1), estimatedOrigin(3), ...
            estimatedNormalForceNow(1) * forceScale, estimatedNormalForceNow(3) * forceScale, 0, ...
            'Color', [0.85 0.15 0.10], 'LineWidth', 1.8, 'MaxHeadSize', 0.8);
        hEstimatedFriction = quiver(axShape, estimatedOrigin(1), estimatedOrigin(3), ...
            estimatedFrictionForceNow(1) * forceScale, estimatedFrictionForceNow(3) * forceScale, 0, ...
            'Color', [0.70 0.20 0.65], 'LineWidth', 1.8, 'MaxHeadSize', 0.8);
        xlabel(axShape, 'x [mm]'); ylabel(axShape, 'z [mm]');
        title(axShape, {sprintf('Frame %d/%d | %s | %s',frameIdx,nVideoFrames,phase,modeLabel), ...
            sprintf('Shape RMSE %.3f mm | forces act on rod',shapeRmseNow)});
        if results.forward.contactArcLength(end)>results.forward.s(end)-1
            title(layout,'Tip contact: contact / independent tip-load split is not identifiable');
        end
        legend(axShape, [hTrueShape, hEstimatedShape, hPlane, hTrueNormal, ...
            hTrueFriction, hEstimatedNormal, hEstimatedFriction], ...
            {'True shape', 'Estimated shape', 'Plane', 'True normal', ...
            'True friction', 'Estimated normal', 'Estimated friction'}, ...
            'Location', 'southoutside');

        axForce = nexttile(layout, 2);
        hold(axForce, 'on'); grid(axForce, 'on'); box(axForce, 'on');
        hTrueNormalHistory = plot(axForce, motion, trueNormal, '-', ...
            'Color', [0.15 0.15 0.15], 'LineWidth', 1.4);
        hEstimatedNormalHistory = plot(axForce, motionHistory, estimatedNormalHistory, '--', ...
            'Color', [0.85 0.15 0.10], 'LineWidth', 1.8);
        hTrueTangentHistory = plot(axForce, motion, trueTangent, '-', ...
            'Color', [0.10 0.45 0.80], 'LineWidth', 1.4);
        hEstimatedTangentHistory = plot(axForce, motionHistory, estimatedTangentHistory, '--', ...
        ...
        legend(axForce, [hTrueNormalHistory, hEstimatedNormalHistory, ...
            hTrueTangentHistory, hEstimatedTangentHistory], ...
            {'True normal magnitude', 'Estimated normal magnitude', ...
            'True friction along motion', 'Estimated friction along motion'}, ...
            'Location', 'best');
...
try
    for frameIdx = 1:nVideoFrames
        ...
        layout = tiledlayout(fig, 1, 2, 'TileSpacing', 'compact', 'Padding', 'compact');
        if ~results.metrics.aloi.finalWithinPaperTransverseLoadAssumption
            title(layout, 'Outside Aloi local-transverse load assumption; raw mismatch only');
        end
        axShape = nexttile(layout, 1);
        hold(axShape, 'on'); grid(axShape, 'on'); box(axShape, 'on'); axis(axShape, 'equal');
        hTrueShape = plot(axShape, pTrue(1, :), pTrue(3, :), '-', ...
...
T.true_normal_Fx_N = results.forward.normalForceResultant(1, :)';
T.true_normal_Fy_N = results.forward.normalForceResultant(2, :)';
T.true_normal_Fz_N = results.forward.normalForceResultant(3, :)';
T.true_friction_Fx_N = results.forward.frictionForceResultant(1, :)';
T.true_friction_Fy_N = results.forward.frictionForceResultant(2, :)';
T.true_friction_Fz_N = results.forward.frictionForceResultant(3, :)';
T.forward_friction_cone_slack_N = results.forward.frictionConeSlack(:);
T.forward_friction_w_min_mm = results.forward.frictionWMin(:);
T.forward_friction_complementarity = results.forward.frictionComplementarity(:);
T.forward_friction_dissipation_Nmm = results.forward.frictionDissipation(:);
T.forward_friction_cone_violation_N = results.forward.frictionConeViolation(:);
T.forward_tangential_displacement_mm = results.forward.frictionTangentialDisplacementNorm(:);
T.forward_friction_direction_violation_Nmm = results.forward.frictionDirectionViolation(:);
...
T.ours_normal_Fx_N = results.ours.normalForceResultant(1, :)';
T.ours_normal_Fy_N = results.ours.normalForceResultant(2, :)';
T.ours_normal_Fz_N = results.ours.normalForceResultant(3, :)';
T.ours_friction_Fx_N = results.ours.frictionForceResultant(1, :)';
T.ours_friction_Fy_N = results.ours.frictionForceResultant(2, :)';
T.ours_friction_Fz_N = results.ours.frictionForceResultant(3, :)';
T.ours_friction_w_min_mm = results.ours.frictionWMin(:);
T.ours_friction_cone_violation_N = results.ours.frictionConeViolation(:);
...
writetable(T, fullfile(cfg.outputDir, 'trajectory.csv'));
end


function writeSummary(results)
cfg = results.config;
path = fullfile(cfg.outputDir, 'summary.txt');
fid = fopen(path, 'w');
...
fprintf(fid, 'Force convention: force exerted by plane on rod, in world coordinates.\n');
fprintf(fid, 'Final contact mode: %s\n', results.ours.complementarityMode{end});
if isfield(results, 'validation') && isfield(results.validation, 'friction')
    audit = results.validation.friction;
...
fprintf(fid, 'RMSE scope below: frames with nonzero true total load.\n');
if isfield(results, 'validation')
    fprintf(fid, 'Shape+environment total-load RMSE over ALL output frames: %.6g N\n', ...
        results.validation.totalForceRmseN);
end
...
function [normalResultant, frictionResultant, coneSlack, minW, ...
        complementarity, dissipation, coneViolation, tangentialDisplacementNorm, ...
        directionViolation] = summarizeContactFriction(contacts, mu)
% Summarize the actual LCP variables retained on each contact record.
normalResultant = zeros(3, 1);
frictionResultant = zeros(3, 1);
coneSlack = inf;
minW = inf;
complementarity = 0;
dissipation = 0;
coneViolation = 0;
tangentialDisplacementNorm = 0;
directionViolation = 0;

for ic = 1:numel(contacts)
    contact = contacts(ic);
    n = unitVector(contact.normal, [0; 0; 1]);
    fn = max(0, contact.normal_force);
    normalForce = n * fn;
    if isfield(contact, 'friction_force')
        frictionForce = contact.friction_force(:);
    else
        frictionForce = zeros(3, 1);
    end
    normalResultant = normalResultant + normalForce;
    frictionResultant = frictionResultant + frictionForce;

    if isfield(contact, 'friction_beta')
        beta = max(0, contact.friction_beta(:));
    else
        beta = zeros(0, 1);
    end
    if isfield(contact, 'friction_w')
        w = contact.friction_w(:);
    else
        w = zeros(size(beta));
    end
    if isfield(contact, 'friction_cone_slack')
        slack = contact.friction_cone_slack;
    else
        slack = mu * fn - sum(beta);
    end
    if isfield(contact, 'contact_displacement')
        dp = contact.contact_displacement(:);
    else
        dp = zeros(3, 1);
    end
    dpTangential = dp - n * (n' * dp);

    coneSlack = min(coneSlack, slack);
    minW = min(minW, min([w; 0]));
    complementarity = max(complementarity, norm(w .* beta));
    dissipation = dissipation + dpTangential' * frictionForce;
    coneViolation = max(coneViolation, sum(beta) - mu * fn);
    tangentialDisplacementNorm = max(tangentialDisplacementNorm, norm(dpTangential));
    directionViolation = max(directionViolation, dpTangential' * frictionForce);
end
if ~isfinite(coneSlack)
    coneSlack = 0;
end
if ~isfinite(minW)
    minW = 0;
